...
def create_reference(path_name, exp_name, groups):
    ...
    reference = os.path.join(path_name, exp_name, f'{exp_name}_reference.csv')
    ...
    exp_path = os.path.join(path_name, exp_name)
...
def rename_labels(reference_path, stages, labels_folder):
    ...
    # Create a subfolder named "all_labels"
    renamed_path = os.path.join(parent_dir, f'all_{labels_folder}')

    # Check if it exists
    if os.path.exists(renamed_path):
        print(f'all_{labels_folder} already exists')
    ...
    for stage in stages:
        ...
        for index, row in reference.iterrows():
            if stage in row['Video']:
                ...
                old_file_path = os.path.join(parent_dir, stage, labels_folder, f'{video_name}_{labels_folder}.csv')
                new_video_name = f'{group_name}_{video_name}_{labels_folder}.csv'
                new_file_path = os.path.join(renamed_path, new_video_name)
...
all_path, groups = rename_labels(reference_path, stages, label_type)
...
"""
We define the functions that will plot the data
"""

#%% Plot all -> See all sessions, one mouse at a time
...
def plot_all(path, name_start, experiment, labels_folder, fps = 25):
    ...
    for filename in os.listdir(path):
        if filename.startswith(name_start) and "TS" in filename:
            ...
            Hab_file_path = TS_file_path.replace("TS", "Hab").replace(f"all_{labels_folder}", "Hab/distances").replace(f"{name_start}_","").replace(f"_{labels_folder}","_distances")
            position_file_path = TS_file_path.replace(f"all_{labels_folder}", "TS/position").replace(f"{name_start}_","").replace(f"_{labels_folder}","_position")
            
            TS = pd.read_csv(TS_file_path)
            TS = calculate_cumulative_sums(TS, time_limit, fps)
            
            TR2 = pd.read_csv(TR2_file_path)
            TR2 = calculate_cumulative_sums(TR2, time_limit, fps)
            
            TR1 = pd.read_csv(TR1_file_path)
            TR1 = calculate_cumulative_sums(TR1, time_limit, fps)
            
            Hab = pd.read_csv(Hab_file_path)
            Hab = calculate_cumulative_sums(Hab, time_limit, fps)
...
def plot_groups(path, name_start, experiment, labels_folder, fps=25):
    ...
    for filename in os.listdir(path):
        if filename.startswith(name_start) and "TS" in filename:
            ...
            TR2_file_path = TS_file_path.replace("TS", "TR2")
            TR1_file_path = TS_file_path.replace("TS", "TR1")
            Hab_file_path = TS_file_path.replace("TS", "Hab").replace(f"all_{labels_folder}", "Hab/distances").replace(f"{name_start}_2","2").replace(f"_{labels_folder}","_distances")
...
plot_experiment(all_path, groups, experiment, label_type)
