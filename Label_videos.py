...
def process_frame(frame, frame_number, L, R, left_sum, right_sum):
    ...
    draw_text(frame, f"Frame: {frame_number + 1} ", 
              pos = (int(frame.shape[0]*0.40), int(frame.shape[1]*0.01)), 
              text_color = (0, 255, 0))
...
def main():
    ...
    while current_frame < len(frame_list):
        ...
        left, right, move = process_frame(frame, current_frame, left, right, left_sum, right_sum)
