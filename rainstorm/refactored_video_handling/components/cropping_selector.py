...
class CroppingSelector:
    ...
    def __init__(self, video_dict: dict):
        ...
        if not self.video_files:
            raise ValueError("CroppingSelector: No video files provided in video_dict.")

        try:
            self.base_image = image_utils.merge_frames(self.video_files)
        except ValueError as e:
            raise ValueError(f"CroppingSelector: Error merging frames - {e}")
